    ...
    if (articleSlug) {
      try {
        ...
        const articleResult = await query(
          `SELECT a.*, c.name as cat_name, au.name as author_name
           FROM articles a
        ...
        const pageUrl = `https://www.mtkenyanews.com/#article/${articleSlug}`;
        const canonical = `https://www.mtkenyanews.com/article/${articleSlug}`;
...
<body>
  ...
  <noscript>
    ...
    // Article by slug - Return HTML with OG tags for crawlers, JSON for API clients
    ...
    if (slugMatch) {
      ...
      // Check if request is from a crawler (user-agent or Accept header hints)
      const userAgent = (req.headers['user-agent'] || '').toLowerCase();
      const isCrawler = /bot|crawler|spider|scraper|curl|wget|facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegram|pinterest|reddit/.test(userAgent);
      const acceptHeader = (req.headers['accept'] || '').toLowerCase();
      
      // Return HTML with OG tags for crawlers or HTML requests
      if (isCrawler || acceptHeader.includes('text/html')) {
        function escapeHtml(text: string): string {
          if (!text) return '';
          const map: { [key: string]: string } = {
            '&': '&amp;',
